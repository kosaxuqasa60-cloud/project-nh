...
// 维度 key：内置为固定字符串（difficulty 等），自定义为��成的 "dim-xxx"。
...
// 精品资源（独立类型）：一份由题目组成的试卷 / 题目包，每道题可带讲解视频。
// 挂载到教材章节（与题库一致），按 市/区/校 级别归属。
export interface Premium extends LeveledResource {
  id: string
  title: string
  subject: string
  description?: string
  items: PremiumQuestion[] // 组成这份试卷 / 题目包的题目（扁平有序列表）
  knowledgePointIds: string[]
  chapterMounts: { textbookId: string; chapterId: string }[]
  coverImage?: string // 封面图
  usedCount?: number
...
// ===================== 精品资源：题目与讲解视频 =====================
// 精品资源被理解为“题目包”：一组直接录入的题目，每题带答案 / 解析，并可附一段讲解视频。

// 讲解视频（模拟上传：url 为本地 object/data URL，fileName 为原文件名）
...
// 精品资源中的一道题：含题型、可选选项、答案 / 解析与讲解视频
export interface PremiumQuestion {
  id: string
  qType: QuestionType // 题型：单选/多选/填空/判断/解答
  label?: string // 题目编号/标识，如 “1” “例1”
  stem: string // 题干
  options?: PremiumOption[] // 单选/多选的选项及正确标记
  answer?: string // 填空/解答/判断的参考答案（判断为 “对”/“错”）
  analysis?: string // 解析
  video?: PremiumVideo // 讲解视频
}

// —— 精品资源统计辅助 ——
export function premiumQuestionCount(p: Pick<Premium, "items">): number {
  return (p.items ?? []).length
}
export function premiumVideoCount(p: Pick<Premium, "items">): number {
  return (p.items ?? []).filter((it) => !!it.video?.url).length
}
